...
> **최종 결정(2026-08-14 저녁): B안(Flighty) 프로덕션 채택 · 라이트 기본 · A안은 랩 비교용 강등 · C안 폐기.**
> 실앱 팔레트(`lib/core/theme/glass_palette.dart`)가 B 토큰의 미러가 됐고, 랩(`/design-test`)은 A/B 비교
> 페이지로 존치(로그인 후 설정에서도 진입). 값 표·문법: `docs/design-direction.md` §0.
