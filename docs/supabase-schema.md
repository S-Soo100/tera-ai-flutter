...
terra-server는 `enclosures` / `devices` / `cameras` / `commands` / `telemetry` / `telemetry_1m` / `alerts` / `motion_clips` 테이블을 정의한다. **현재 Flutter 앱이 연동한 것은 `devices` / `telemetry` / `commands` / `cameras` / `motion_clips`(비디오·썸네일·활동량)**이며(+ petcam-lab `behavior_logs` 분류라벨 읽기[owner SELECT RLS, 2026-07-08] · 신규 `clip_favorites` 즐겨찾기 클라우드 동기화), 나머지는 명세만 존재하고 앱 미연동 상태다.
