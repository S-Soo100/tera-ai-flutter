...
| 3-3 최신 날짜 | 하이라이트는 상세 목록의 최신 대표 영상 시각을 사용해 공개 메타가 없어도 날짜를 표시. 북마크는 저장시각 기준 `오늘/어제/N일 전`만 표시 | 없음 |
...
- 카메라 홈 최신 날짜 후속 보정: 실제 로그인 데이터에서 공개 메타가 없던 하이라이트가 `업데이트 오늘`로 표시되고, 북마크는 `오늘`로 `업데이트` 접두어가 제거됨을 iPhone 17 시뮬레이터에서 확인했다. 14일 경계는 위젯 회귀 테스트로 확인했다.
