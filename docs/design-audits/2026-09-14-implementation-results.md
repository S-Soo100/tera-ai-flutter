...
작업 공간 `.worktrees/final-design`, 브랜치 `codex/final-design-implementation`, 최종 후속 보정 버전 `0.100.2+194`.
...
| 3-3 최신 날짜 | 하이라이트는 최신 서버 `day_key`를 상세와 같은 밤 묶음 기준으로 해석해 최신을 `어젯밤`으로 표시. 북마크는 저장시각 기준 `오늘/어제/N일 전`만 표시 | 없음 |
...
4. 주간 대표값/막대색 혼용 → Figma 원본 레이어를 재확인해 온도 `F85478`, 습도 `00B2F3`로 통일하고, 일반 막대 `626262`, 최저 막대 `A9B3BE`를 적용.
...
- 홈 라이브 면에 별도로 남아 있던 `LIVE`/연결 끊김 상태 배지도 제거했다. 라이브 시각·카메라명·확대 버튼은 유지한다.
...
- 카메라 홈 최신 날짜 후속 보정: 하이라이트는 오늘 새벽 영상의 촬영일이 아니라 서버 밤 묶음 `day_key`를 사용한다. 실제 로그인 데이터로 iPhone 17 시뮬레이터에서 `하이라이트 · 어젯밤`, `북마크 영상 · 오늘`을 확인했다. 밤 8시가 지나도 전날 묶음은 `어젯밤`으로 유지하며, 14일 경계는 위젯 회귀 테스트로 확인했다.
