...
2. **별도 기능 후보:** 그룹 상세 ‘이 그룹 삭제’ 버튼. 그룹 제거 후 구성원 처리와 전용 서버 계약을 먼저 확정하며, 기존 단일 구성원 제외를 반복 호출하는 방식으로 구현하지 않는다.
3. **별도 상태 후보:** 카메라 미연결 MyCre의 연결 안내·‘카메라와 그룹만들기’ CTA·빈 격자. Figma0h0m은 미연결 전용 표현으로 쓸지 확정하며, 관측 부족/오류의 -- 정책을 전역 변경하지 않는다.
4. **검증 예시 추가:** 그룹에는 사육장·개체, 그룹 없음에는 카메라가 있는 상태. ‘그룹의 장점 안내’ 메모는 원문에서 나중 추가로 적혀 있어 이번 범위에 자동 포함하지 않는다.

위 내용은 확인된 신규 입력과 영향 기록이며, 기존9항목 외 구현 승인을 받았다는 뜻이 아니다.
