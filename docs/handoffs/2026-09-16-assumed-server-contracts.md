...
**2026-09-16 통합 회신 수신** ([원문](../references/2026-09-16-backend-delivery-reply.md)). 대조 결과를 "회신 결과" 열에 적었다. **결정 답신([reply-2](2026-09-16-lee-gwanhun-reply-2-decisions.md))과 답신 1·SQL 초안 폴더는 2026-09-16 사용자가 직접 전달 완료.** secret은 사용자가 별도 채널로 전달(앱 팀 미관여). 다음 대기: 소프트 해제 배포 통보, RPC 초안 검토 결과, 히터 400 적용, skipped는 서버도 구현 완료(스위치 `PUSH_EVENT_SKIPPED_ENABLED` 꺼짐) → [발송 시작 신호](2026-09-16-lee-gwanhun-skipped-go-signal.md) 전달 대기(사용자). 히터는 예약 400 + 직접 INSERT `rejected/unsupported_action`으로 서버 차단 완료. 아직 사용자 결정이 필요한 줄은 굵게 표시.
...
| 6 | ⏸️ **초안 5개를 아직 못 읽음**("저장소 밖, 경로/파일 필요"). 트리거 방식 통합 동의. 그룹 이름 UNIQUE `(owner_id, btrim(name))`는 적용 완료·409 | 폴더 수령 확인, 검토 후 별도 회신 예정. 트리거 방식 통합 재동의. `redesign_unlink_device_v1`은 REST로 대체됐으니 0A000 가드로 두거나 제거 가능 |
