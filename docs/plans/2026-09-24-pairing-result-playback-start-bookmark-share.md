...
> 상태: **구현 완료·main 병합(2026-09-24).** 사용자 결정: 2번 B안, 3번 ①체크 둠 ②바텀 모달 ③게시 후 커뮤니티 탭 유지.
> - 2-B → 0.133.15+344 (`a96afe2`) · 1 → 0.133.16+345 (`70f064c`) · 3 → 0.134.0+346 (`5deb1b4`)
> - 1번은 실기기 재현 로그 없이 구현했다. **S21+로 사육장+카메라 동시 등록을 다시 해 "연결 확인 중 → 연결 완료"를 확인하는 것이 남았다.**
> - **코드리뷰 3건 반영 → 0.134.1+347.** 1번 판정 근거를 좁혔다: 서버 감시는 **`CONNECT` 뒤 BLE 끊김·무응답**(카메라 재부팅으로 `WIFI_OK` 유실)일 때만. 기기가 `WIFI_FAIL`을 준 경우(가설 2)와 `CONNECT` 전 끊김은 즉시 실패 — 옛 Wi-Fi 하트비트가 계속 와서 서버 감시가 오판한다. 접속 판정은 영수증 뒤 읽은 `last_seen_at` 기준값보다 새로운 값만(시계 오차 무관). 남는 한계: `CONNECT` 뒤 새 비밀번호가 틀렸는데 펌웨어가 옛 Wi-Fi로 되돌아가 하트비트를 재개하면 여전히 '연결 완료'로 읽힌다 — 펌웨어 동작 확인 필요. 실기기 재현 시 `[device-add] camera CONNECT(wifi-only) -> …` / `ended: … connectSent=…` 로그로 어느 경로였는지 갈린다.
