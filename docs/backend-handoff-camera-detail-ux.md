...
## 요청 4 — 크레캠 영상목록 분류 라벨 접근 (behavior_logs)

### 문제
크레캠 세부 "비디오 기록"의 분류 필터/칩이 전부 "미분류"·특정 분류 선택 시 0건. 원인: `motion_clips`엔 `action` 컬럼이 없고, 실제 VLM/human 분류는 `behavior_logs`(action·clip_id·source·verified)에 있는데 **`behavior_logs`는 RLS 켜짐 + 정책 0개 = 앱(authenticated) 직접 읽기 전면 차단**(실측: `pg_policies` 0건). 리포트 하이라이트는 서버(service-role)가 읽어 내려주므로 정상.

### 확인된 사실 (실 DB, 2026-07-08)
- motion_clips와 매칭되는 라벨: `vlm` 이동 1,079 · 탈피 54 · 음수 8 · 핸드피딩 2 · 자율급여 1 + `human` 일부. **데이터는 있고 앱만 못 읽음.**
- `behavior_logs.clip_id`(=camera_clips.id)는 미러라 `motion_clips.id`와 **동일 UUID** → `behavior_logs.clip_id = motion_clips.id` 조인 성립.

### 요청 (택1)
**A. behavior_logs owner SELECT RLS 정책** (간단):
```sql
create policy "owner reads own clip labels" on behavior_logs
  for select using (
    exists (select 1 from motion_clips mc
            where mc.id = behavior_logs.clip_id and mc.owner_id = auth.uid()));
```
→ 앱이 **자기 클립의 라벨만** 읽음. (raw `reasoning`/오탐 라벨 노출이 부담이면 B.)

**B. terra-api 라벨 배치 엔드포인트** (curated): `GET /clips/labels?ids=…` → `{clip_id: action}`(클립당 대표 라벨 1개, `verified`/`human` 우선). 서버가 behavior_logs를 service-role로 읽어 내려줌.

### 앱 이관
둘 중 하나 열리면 앱은 `kClipClassificationEnabled`(`lib/features/my_cage/data/motion_clip_repository.dart`)를 **true로 켜기만** 하면 `listByCamera`가 라벨을 조인(A) 하거나 엔드포인트로 받아(B) 카드 칩·분류 필터를 실동작시킨다(조인 코드 이미 대기). 현재는 깨진 필터/미분류 칩을 임시로 숨긴 상태(라벨 있는 클립만 칩 표시).
