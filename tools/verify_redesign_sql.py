...
for name in ['20260915_assignment_history.sql', '20260915_redesign_groups.sql',
             '20260916_relationship_triggers.sql',
...
parts += [(root / 'test/sql/redesign_contract_assertions.sql').read_text(),
          (root / 'test/sql/redesign_delete_group_assertions.sql').read_text(),
          (root / 'test/sql/redesign_unlink_trigger_assertions.sql').read_text(), 'ROLLBACK;']
