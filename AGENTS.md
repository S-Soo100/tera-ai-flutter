...
### 역할 매핑
- **Designer**: 메인 Codex (opus) -- 분석, 설계, Phase 전환 판단
- **Implementer**: flutter-dev (sonnet) -- Dart 코드 구현, 버그 수정

### 사용자 안내 규칙
작업 요청 시, 메인 Codex는 **트랙 판단 결과를 먼저 알려준다**:
```
...
상세 라우팅 트리: `.Codex/rules/tera-ai-caof.md`
...
2회 실패: Designer(메인 Codex) 원인 재분석 -> 다른 접근법
...
- `flutter build`는 Codex 안에서 직접 실행 가능 (리소스 경합 낮음, Unity와 다름).
