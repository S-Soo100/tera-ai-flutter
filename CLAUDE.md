...
> **분무**: `mist` + `payload.duration_ms`. 홈 타일 탭은 제어 시트를 열고 **"1회 분사 시작" → 2초 실행 취소 창 → 3000ms 분사**(2026-09-16 Figma 재설계, `mistWithUndo` — 2026-09-07의 타일 즉시 분사는 대체, 사용자 확인 대기). 타일 표기는 꺼짐/작동 중=릴레이 텔레메트리+5초 잠금. 예약 편집기의 1/2/3초 선택은 유지. 앱은 지속시간만 보내고 **OFF는 보내지 않는다** — 펌웨어가 스스로 끈다. 과거 `relay_toggle` 144건도 계속 분무로 해석한다(`MistDuration`, `home_mist_*`).
