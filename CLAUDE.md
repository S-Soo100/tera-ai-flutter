...
> **분무**: `mist` + `payload.duration_ms`. **홈 타일은 무조건 3000ms 즉시 분사**(2026-09-07 사용자 지시 — 시간 선택 시트·mistDurationProvider 폐지, 타일 표기는 꺼짐/켜짐=릴레이 텔레메트리+5초 잠금). 예약 편집기의 1/2/3초 선택은 유지. 앱은 지속시간만 보내고 **OFF는 보내지 않는다** — 펌웨어가 스스로 끈다. 과거 `relay_toggle` 144건도 계속 분무로 해석한다(`MistDuration`, `home_mist_*`).
