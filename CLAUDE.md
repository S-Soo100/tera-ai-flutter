...
- **백엔드 핸드오프 2026-09-07 (mist 블랙아웃·명령 유실)**: `docs/backend-handoff-2026-09-07-mist-blackout.md` — 분무 실행 시 기기가 ~25초 침묵(텔레메트리 중단+명령 수신 불가, 실DB 실증). 그 창의 명령은 영영 `sent`로 남는다(서버에 sent 만료 없음). 앱은 ACK 15초 유예(`kCommandAckGrace`)로 완화 — 근본 해결은 펌웨어 대기
