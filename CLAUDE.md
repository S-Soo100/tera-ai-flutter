...
- **Implementer**: flutter-dev -- Dart 코드 구현 (Critical에서 투입, 아래 프로젝트 서브에이전트 모델 규칙 적용)
- Standard는 메인이 역할 겸임 (분석서 → 합의 → 직접 구현)

### 프로젝트 서브에이전트 모델 (2026-09-15 사용자 지정)

- 이 프로젝트에서 서브에이전트를 호출할 때는 역할과 관계없이 **`gpt-5.6-terra` / reasoning effort `medium`**을 명시한다. 메인 에이전트 모델은 변경하지 않는다.
- `spawn_agent`는 모델 오버라이드를 전달할 수 있도록 `fork_turns="none"` 또는 필요한 최근 턴 수를 지정하고, 작업 맥락·담당 범위·결정사항을 프롬프트에 전달한다.
- 지정 모델을 사용할 수 없으면 임의로 다른 모델로 대체하지 말고 사용자에게 알린다.
- 이 규칙은 기존 문서의 ‘모델 상속 기본’보다 우선하며, 서브에이전트 사용 여부는 기존 CAOF 트랙 규칙을 따른다.
