...
- **알림 권한 프리팝업 (2026-09-18, Figma 권한 요청 `1179:4464`):** 로그인 직후 일괄 시트는 폐지. `PushConsentFlow`(`notification/domain/push_consent_flow.dart`)가 선택×시스템 권한 분기를, `push_pre_popup.dart`가 팝업을 맡는다. 시점 = 하이라이트: 카메라 탭이 보이고 카메라+클립 1개 이상(`CrecamScreen._maybeAskHighlightPush`) / 커뮤니티: 게시 성공 직후(`compose_screen`). 주제별 1회(Hive `push_prompt_asked_<topic>`), 받지 않기=해당 토글 끔. iOS(FCM 미지원 `unavailable`)도 팝업은 뜨고 토글만 바꾼다(사용자 결정). 전역 글자 1.15배라 제목은 줄마다 축소, 본문은 어절 단위 줄바꿈(`keepAllWords`).
