...
### UI/테마
- **현행 공통색은 Figma `vivnanaut`의 VIVA 변수 (2026-09-14 사용자 스크린샷)** — 원본 14개는 `lib/core/theme/viva_colors.dart`의 `VivaColors`, 라이트 역할 매핑은 `GlassPalette.light`·`AppTheme.light`. 바닥 `#FAFAFA`, 버튼 면 `#F4F4F4`, 선 `#E3E3E3`, 글자 `#1E1E1E/#3C3C3C/#626262/#949090`, 비활성 아이콘 `#B4AEAE`. 기존 `bodySecondary`=Labels/Tertiary, `textTertiary`=Labels/Quaternary. Main/Sub의 Dark/light는 브랜드 변형이며 앱 테마 모드가 아니다. 기기·차트 전용색과 다크벌은 기존 값 유지. 상세 매핑·출처 범위: `docs/design-system-viva-colors.md`. 화면은 `context.glass`/`Theme.of(context)` 역할색 사용. **팔레트 필드 추가 시 선언·생성자·dark·light·copyWith·lerp 6곳 동시 수정**.
