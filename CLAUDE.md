...
|-----------|---------|------|------------|
| `/home` (탭1) | home | HomeScreen — **단일 스크롤**(2026-09-02): 헤더(세트 드롭다운/`[+]` 추가 메뉴/`[사람]`=계정) + 라이브(`TopFixedArea`) + 온습도 요약 카드(`EnvSummaryCard`, 탭→`/env-detail`) + 제어 그리드(`CageControlGrid` 4타일 — 히터팬은 2026-09-04 사용자 지시로 **숨김**(배선 `_heaterTile` 존치, ⚠️ 숨긴 동안 히터 OFF 진입점 없음 — 복귀·별도 진입점 후속), 냉각팬은 API 없어 미배선 "준비 중") + 일정 설정. 서브탭·타임라인·개체 프로필 분기 폐기 | 사육장 세트(`EnclosureSet`) + `homeTodayExtremesProvider`(오늘 자정~) |
