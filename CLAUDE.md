...
> **LED 밝기는 없다**: 현 보드 LED는 PWM이 아니라 on/off 릴레이다(백엔드 회신 2026-08-12). 슬라이더를 걷어내고 켜기/끄기 시트로 바꿨다. MOSFET 보드 계약(`led_on`+`brightness`)은 생겼지만(2026-08-14) **보드 타입 구분 필드가 없어** 문의 대기 — 그 전까지 밝기 UI를 되살리지 말 것.
