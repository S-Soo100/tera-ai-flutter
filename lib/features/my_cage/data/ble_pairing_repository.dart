...
/// 저장된 서버 자격증명 삭제 완료 (`UNPAIR_OK`, 펌웨어 요청서 2026-09-17 §2-2).
class BleUnpairOk extends BlePairingEvent {}

/// JWT 수신 시작 확인 (`JWT_BEGIN_OK`).
class BleJwtBeginOk extends BlePairingEvent {}

/// 서버 등록 실패 (`PAIR_FAIL <사유>`) — 사유: `401`/`timeout`/`no_name`/`server_5xx`.
class BlePairFail extends BlePairingEvent {
  BlePairFail(this.reason);
  final String reason;
}
...
class BlePairingRepository {
  ...
  void _dispatchTxMessage(String msg) {
    ...
    if (msg == 'UNPAIR_OK') {
      _eventController.add(BleUnpairOk());
      return;
    }
    if (msg == 'JWT_BEGIN_OK' || msg.startsWith('JWT_BEGIN_OK ')) {
      _eventController.add(BleJwtBeginOk());
      return;
    }
    if (msg == 'PAIR_FAIL' || msg.startsWith('PAIR_FAIL ')) {
      _eventController.add(BlePairFail(msg.substring(9).trim()));
      return;
    }
