/// 사육장 액추에이터 제어 동작 모음.
///
/// **제어 진입점이 둘이라 여기 하나로 모은다** — 서브탭의 2x2 그리드
/// (`QuickControlGrid`)와 라이브 아래 컴팩트 바(`LiveControlBar`).
/// 특히 히터는 과열 시 개체 폐사로 이어지는 유일한 액추에이터라, 두 진입점이
/// **같은 안전 확인 플로우를 반드시 공유해야 한다.** 위젯마다 복붙하면
/// 한쪽만 고쳐지고 다른 쪽이 안전장치 없이 남는다.
library;
