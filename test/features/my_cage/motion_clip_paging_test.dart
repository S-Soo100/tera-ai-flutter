...
void main() {
  test('동일 촬영시각 241개를 복합 커서로 누락 없이 순회한다', () async {
    ...
    final rows = List.generate(
        ...
        (i) => {
              'id':
                  '00000000-0000-0000-0000-${(241 - i).toString().padLeft(12, '0')}',
              'camera_id': 'camera-a',
              'started_at': stamp.toIso8601String(),
              'duration_sec': 8,
            });
    final requests = <Uri>[];
    final client = SupabaseClient('https://example.test', 'anon',
        httpClient: MockClient((request) async {
      if (!request.url.path.endsWith('motion_clips'))
    ...
    final repo = MotionClipRepository(
        supabase: client,
        terraApiUrl: 'https://example.test',
        tokenProvider: () async => null);
    MotionClipCursor? cursor;
    final ids = <String>[];
    for (var i = 0; i < 5; i++) {
      final page = await repo.listPage(
          (ownerId: 'owner-a', cameraId: 'camera-a', range: null),
          before: cursor);
