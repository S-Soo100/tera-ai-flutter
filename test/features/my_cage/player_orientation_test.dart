...
void main() {
  ...
  test('세로 진입, 명시적 가로 전환, 종료 시 세로 복원 순서를 보장한다',() async {
    ...
    await controller.toggle();expect(controller.state,true);
    controller.dispose();await PlayerOrientationController.settled;
    final orientations=calls.where((call)=>call.method=='SystemChrome.setPreferredOrientations')
      .map((call)=>call.arguments).toList();
    expect(orientations,[['DeviceOrientation.portraitUp'],
      ['DeviceOrientation.landscapeLeft','DeviceOrientation.landscapeRight'],
      ['DeviceOrientation.portraitUp']]);
    expect(calls.where((call)=>call.method=='SystemChrome.setEnabledSystemUIOverlays').last.arguments,
  ...
  test('방향 요청 처리 중 닫아도 마지막 요청은 세로 복원이다',() async {
    ...
    addTearDown(()=>TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
      .setMockMethodCallHandler(SystemChannels.platform,null));
    final controller=PlayerOrientationController();
    controller.toggle();controller.dispose();
    await PlayerOrientationController.settled;
