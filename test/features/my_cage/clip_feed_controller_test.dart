...
MotionClip clip(String id) => MotionClip(
    id: id,
    cameraId: 'c',
...
MotionClipPage page(List<String> ids, {bool more = false}) => (
      ...
      hasMore: more,
      nextCursor:
          more ? (startedAt: clip(ids.last).startedAt, id: ids.last) : null
    );
void main() {
  test('추가 로드 중 기존 항목 유지·중복 요청 방지·ID 중복 제거', () async {
    ...
    final controller = ClipFeedController((cursor) async {
      calls++;
      return cursor == null ? page(['3', '2'], more: true) : pending.future;
    });
  ...
  test('갱신 실패에도 이전 목록을 유지하고 다시 시도할 수 있다', () async {
    ...
    await controller.refresh();
    expect(controller.state.pageError, isNull);
  });
