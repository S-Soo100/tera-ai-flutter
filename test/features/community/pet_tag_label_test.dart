...
void main() {
  ...
  test('birthDate 없으면 나이 생략', () {
    expect(petTagLabel(morph: '릴리화이트', sex: 'female', now: now), '릴리화이트 여아');
  });
  test('모프 없으면 건너뛴다', () {
    expect(
        petTagLabel(sex: 'female', birthDate: DateTime(2024, 5, 1), now: now),
        '2살 여아');
  });
  test('전부 없으면 null', () {
  ...
  test('생후 0개월(당월)은 나이 생략', () {
    expect(
        petTagLabel(sex: 'unknown', birthDate: DateTime(2026, 8, 20), now: now),
        '아가');
  });
}
